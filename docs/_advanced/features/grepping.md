...
Grepping is a passive scan with patterns during the XSS scanning flow. By default, grepping is enabled in XSSFox.

## Built-in Patterns

XSSFox includes a set of built-in patterns for grepping. Here are some examples:
...
```go
{
    ...
    "xssfox-github-access-token":   "[a-zA-Z0-9_-]*:[a-zA-Z0-9_\\-]+@github\\.com*",
    ...
    "xssfox-error-postgresql":      "(PostgreSQL.*ERROR|Warning.*\\Wpg_.*|valid PostgreSQL result|Npgsql\\.|PG::SyntaxError:|org\\.postgresql\\.util\\.PSQLException|ERROR:\\s\\ssyntax error at or near)",
    ...
    "xssfox-error-psql7":  "ERROR:\\s\\ssyntax error at or near",
...
```bash
xssfox url https://google.com --skip-grepping
```
...
```bash
xssfox url https://google.com --grep grep_pattern.json
```
...
You can find a sample custom grepping file [here](https://github.com/JGPatelOfficial/xssfox/blob/main/samples/sample_grep.json).
