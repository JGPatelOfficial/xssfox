	...
	tests := []struct {
		...
		},
		{
			...
			want: func() http.RoundTripper {
				...
				harWriter, err := har.NewWriter(file, &har.Creator{Name: "xssfox", Version: "v2.0.0"})
				if err != nil {
...
func TestCustomTransportWithXSSFox(t *testing.T) {
	// Create a test server that returns the headers it received
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	// Create XSSFox options with the custom transport
	options := model.Options{
		CustomTransport: customTransport,
		Timeout:         10,
	}

	// Get the transport from XSSFox
